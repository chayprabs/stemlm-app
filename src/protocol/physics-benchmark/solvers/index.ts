...
export const verifyQ26: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    ...
    expectApprox(ctx, 'reduced energy x', solverVal(values, 'x'), { rel: 0.02, abs: 0.02 });
    expectApprox(ctx, 'Fermi-Dirac occupancy', solverVal(values, 'fFD'), { rel: 0.03, abs: 5e-4 });
    expectApprox(ctx, 'Bose-Einstein occupancy', solverVal(values, 'fBE'), { rel: 0.03, abs: 5e-4 });
    expectApprox(ctx, 'Maxwell-Boltzmann occupancy', solverVal(values, 'fMB'), { rel: 0.03, abs: 5e-4 });
    expectApprox(ctx, 'relative quantum difference', solverVal(values, 'delta'), { rel: 0.05, abs: 0.005 });
  });

export const verifyQ27: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ27();
    expectApprox(ctx, 'angular frequency', solverVal(values, 'omega'), { rel: 0.02 });
    expectApprox(ctx, 'centripetal acceleration', solverVal(values, 'a'), { rel: 0.03 });
    expectApprox(ctx, 'Larmor power', solverVal(values, 'power'), { rel: 0.06 });
    expectApprox(ctx, 'radiated energy per cycle', solverVal(values, 'dE'), { rel: 0.06 });
    expectApprox(ctx, 'photon momentum scale', solverVal(values, 'pGamma'), { rel: 0.08 });
  });

export const verifyQ28: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ28();
    expectApprox(ctx, 'retarded-time delay', solverVal(values, 'tDelay'), { rel: 0.03 });
    expectApprox(ctx, 'wave number k', solverVal(values, 'k'), { rel: 0.02, abs: 0.05 });
    expectApprox(ctx, 'far-zone E amplitude', solverVal(values, 'e0'), { rel: 0.05 });
    expectApprox(ctx, 'far-zone B amplitude', solverVal(values, 'b0'), { rel: 0.05 });
    expectApprox(ctx, 'average Poynting flux', solverVal(values, 'sAvg'), { rel: 0.08 });
  });

export const verifyQ29: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ29();
    expectApprox(ctx, 'Lorentz gamma', solverVal(values, 'gamma'), { rel: 0.02, abs: 0.003 });
    expectApprox(ctx, 'lab-frame energy', solverVal(values, 'energy'), { rel: 0.03, abs: 0.005 });
    expectApprox(ctx, 'lab-frame pc', solverVal(values, 'pc'), { rel: 0.03, abs: 0.005 });
    expectApprox(ctx, 'mass-shell invariant', solverVal(values, 'invariant'), { rel: 0.04, abs: 0.004 });
    expectApprox(ctx, 'boosted-frame energy', solverVal(values, 'ePrime'), { rel: 0.04, abs: 0.006 });
  });

export const verifyQ30: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ30();
    expectApprox(ctx, 'CG coefficient sqrt(2/3)', solverVal(values, 'cgA'), { rel: 0.02, abs: 0.003 });
    expectApprox(ctx, 'CG coefficient sqrt(1/3)', solverVal(values, 'cgB'), { rel: 0.02, abs: 0.003 });
    expectApprox(ctx, 'probability weight 2/3', solverVal(values, 'pA'), { rel: 0.02, abs: 0.003 });
    expectApprox(ctx, 'probability weight 1/3', solverVal(values, 'pB'), { rel: 0.02, abs: 0.003 });
    expectApprox(ctx, 'orthogonality dot product', solverVal(values, 'orth'), { rel: 0.02, abs: 0.003 });
  });

export const verifyQ31: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ31();
    expectApprox(ctx, 'optimal variational alpha', solverVal(values, 'alphaStar'), { rel: 0.03 });
    expectApprox(ctx, 'trial energy in eV', solverVal(values, 'eTrialEv'), { rel: 0.05, abs: 0.1 });
    expectApprox(ctx, 'minimum energy in joules', solverVal(values, 'eMinJ'), { rel: 0.05 });
    expectApprox(ctx, 'minimum energy in eV', solverVal(values, 'eMinEv'), { rel: 0.04, abs: 0.1 });
    expectApprox(ctx, 'optimization improvement', Math.abs(solverVal(values, 'gainEv')), { rel: 0.3, abs: 0.05 });
  });

export const verifyQ32: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ32();
    expectApprox(ctx, 'well ground energy E1', solverVal(values, 'e1Ev'), { rel: 0.03, abs: 0.01 });
    expectApprox(ctx, 'well first excited energy E2', solverVal(values, 'e2Ev'), { rel: 0.03, abs: 0.02 });
    expectApprox(ctx, 'boson two-particle energy', solverVal(values, 'eBoson'), { rel: 0.03, abs: 0.02 });
    expectApprox(ctx, 'fermion two-particle energy', solverVal(values, 'eFermion'), { rel: 0.03, abs: 0.03 });
    expectApprox(ctx, 'boson-fermion energy gap', solverVal(values, 'deltaEv'), { rel: 0.04, abs: 0.03 });
  });

export const verifyQ33: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ33();
    expectApprox(ctx, 'perturbation matrix element', solverVal(values, 'vfi'), { rel: 0.05 });
    expectApprox(ctx, 'Fermi-golden-rule transition rate', solverVal(values, 'rate'), { rel: 0.08 });
    expectApprox(ctx, 'transition lifetime', solverVal(values, 'tau'), { rel: 0.08 });
    expectApprox(ctx, 'photon momentum emission scale', solverVal(values, 'pGamma'), { rel: 0.08 });
  });

export const verifyQ34: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ34();
    expectApprox(ctx, 'critical temperature Tc', solverVal(values, 'tc'), { rel: 0.03, abs: 1.0 });
    expectApprox(ctx, 'reduced exchange A', solverVal(values, 'a'), { rel: 0.03, abs: 0.005 });
    expectApprox(ctx, 'reduced field h', solverVal(values, 'h'), { rel: 0.05 });
    expectApprox(ctx, 'linearized magnetization m', solverVal(values, 'm'), { rel: 0.05 });
    expectApprox(ctx, 'per-spin field energy', solverVal(values, 'fieldEnergy'), { rel: 0.08 });
  });

export const verifyQ35: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ35();
    expectApprox(ctx, 'equipartition thermal energy U', solverVal(values, 'u'), { rel: 0.03 });
    expectApprox(ctx, 'per-particle thermal energy', solverVal(values, 'uPerParticle'), { rel: 0.03 });
    expectApprox(ctx, 'rms speed', solverVal(values, 'vRms'), { rel: 0.03 });
    expectApprox(ctx, 'rms momentum', solverVal(values, 'pRms'), { rel: 0.04 });
    expectApprox(ctx, 'virial potential average', solverVal(values, 'vAvg'), { rel: 0.03 });
    expectApprox(ctx, 'virial total energy', solverVal(values, 'eTotal'), { rel: 0.03 });
  });

export const verifyQ36: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ36();
    expectApprox(ctx, 'reciprocal lattice constant a*', solverVal(values, 'aStar') * 1e-9, { rel: 0.03, abs: 0.2 });
    expectApprox(ctx, '|G111| in nm^-1', solverVal(values, 'g111') * 1e-9, { rel: 0.03, abs: 0.2 });
    expectApprox(ctx, '|G200| in nm^-1', solverVal(values, 'g200') * 1e-9, { rel: 0.03, abs: 0.2 });
    expectApprox(ctx, 'd111 spacing in nm', solverVal(values, 'd111Nm'), { rel: 0.03, abs: 0.002 });
    expectApprox(ctx, 'd200 spacing in nm', solverVal(values, 'd200Nm'), { rel: 0.03, abs: 0.002 });
  });

export const verifyQ37: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ37();
    expectApprox(ctx, 'Fermi wave vector', solverVal(values, 'kF'), { rel: 0.03 });
    expectApprox(ctx, 'Fermi energy in eV', solverVal(values, 'eFEv'), { rel: 0.03, abs: 0.05 });
    expectApprox(ctx, 'Fermi velocity', solverVal(values, 'vF'), { rel: 0.03 });
    expectApprox(ctx, 'DOS at EF in eV^-1 m^-3', solverVal(values, 'gEv'), { rel: 0.06 });
    expectApprox(ctx, 'kT/EF degeneracy ratio', solverVal(values, 'ratio'), { rel: 0.08, abs: 2e-4 });
  });

export const verifyQ38: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ38();
    expectApprox(ctx, 'zone-boundary wave number', solverVal(values, 'kBz'), { rel: 0.03 });
    expectApprox(ctx, 'zone-boundary free-electron energy', solverVal(values, 'e0Ev'), { rel: 0.03, abs: 0.05 });
    expectApprox(ctx, 'lower split branch energy', solverVal(values, 'eMinus'), { rel: 0.03, abs: 0.05 });
    expectApprox(ctx, 'upper split branch energy', solverVal(values, 'ePlus'), { rel: 0.03, abs: 0.05 });
    expectApprox(ctx, 'band gap', solverVal(values, 'gap'), { rel: 0.02, abs: 0.01 });
  });

export const verifyQ39: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ39();
    expectApprox(ctx, 'volume term', solverVal(values, 'bv'), { rel: 0.02, abs: 0.5 });
    expectApprox(ctx, 'surface term', solverVal(values, 'bs'), { rel: 0.03, abs: 0.5 });
    expectApprox(ctx, 'Coulomb term', solverVal(values, 'bc'), { rel: 0.03, abs: 0.5 });
    expectApprox(ctx, 'total binding energy MeV', solverVal(values, 'bMeV'), { rel: 0.03, abs: 0.7 });
    expectApprox(ctx, 'binding energy per nucleon', solverVal(values, 'bPerA'), { rel: 0.03, abs: 0.05 });
  });

export const verifyQ40: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ40();
    expectApprox(ctx, 'parent population NA', solverVal(values, 'nA'), { rel: 0.03 });
    expectApprox(ctx, 'daughter population NB', solverVal(values, 'nB'), { rel: 0.03 });
    expectApprox(ctx, 'stable product population NC', solverVal(values, 'nC'), { rel: 0.03 });
    expectApprox(ctx, 'activity of parent in Bq', solverVal(values, 'aA_bq'), { rel: 0.03, abs: 0.2 });
    expectApprox(ctx, 'activity of daughter in Bq', solverVal(values, 'aB_bq'), { rel: 0.03, abs: 0.2 });
  });

export const verifyQ41: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ41();
    expectApprox(ctx, 'Michelson fringe shift count', solverVal(values, 'fringes'), { rel: 0.03, abs: 0.02 });
    expectApprox(ctx, 'phase shift', solverVal(values, 'phaseShift'), { rel: 0.03, abs: 0.05 });
    expectApprox(ctx, 'Fabry-Perot FSR', solverVal(values, 'fsr'), { rel: 0.03 });
    expectApprox(ctx, 'Fabry-Perot finesse', solverVal(values, 'finesse'), { rel: 0.04, abs: 0.2 });
    expectApprox(ctx, 'Fabry-Perot linewidth', solverVal(values, 'linewidth'), { rel: 0.04 });
  });

export const verifyQ42: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ42();
    expectApprox(ctx, 'first-order grating position', solverVal(values, 'y1') * 100, { rel: 0.04, abs: 0.1 });
    expectApprox(ctx, 'first diffraction minimum', solverVal(values, 'yMin1') * 100, { rel: 0.04, abs: 0.1 });
    expectApprox(ctx, 'missing-order rule ratio d/a', solverVal(values, 'missingSpacing'), { rel: 0.02, abs: 0.02 });
    expectApprox(ctx, 'visible central orders', solverVal(values, 'centralVisibleOrders'), { rel: 0.02, abs: 0.1 });
  });

export const verifyQ43: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ43();
    expectApprox(ctx, 'laser photon energy in eV', solverVal(values, 'photonEnergyEv'), { rel: 0.03, abs: 0.02 });
    expectApprox(ctx, 'three-level threshold fraction', solverVal(values, 'fracThree') * 100, { rel: 0.03, abs: 0.5 });
    expectApprox(ctx, 'four-level threshold fraction', solverVal(values, 'fracFour') * 100, { rel: 0.04, abs: 0.05 });
    expectApprox(ctx, 'three-vs-four threshold ratio', solverVal(values, 'fractionRatio'), { rel: 0.04, abs: 1.0 });
    expectApprox(ctx, 'slope-efficiency output power', solverVal(values, 'pOut'), { rel: 0.03, abs: 0.05 });
  });

export const verifyQ44: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ44();
    expectApprox(ctx, 'Green function value G(x,x0)', solverVal(values, 'g'), { rel: 0.03, abs: 0.005 });
    expectApprox(ctx, 'potential at x=0.60 m', solverVal(values, 'phi'), { rel: 0.03, abs: 0.3 });
    expectApprox(ctx, 'left electric field branch', solverVal(values, 'eLeft'), { rel: 0.03, abs: 1.0 });
    expectApprox(ctx, 'right electric field branch', solverVal(values, 'eRight'), { rel: 0.03, abs: 1.0 });
    expectApprox(ctx, 'field jump across source', solverVal(values, 'jump'), { rel: 0.03, abs: 1.5 });
  });

export const verifyQ45: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ45();
    expectApprox(ctx, 'first stress invariant I1', solverVal(values, 'i1'), { rel: 0.02, abs: 0.5 });
    expectApprox(ctx, 'second stress invariant I2', solverVal(values, 'i2'), { rel: 0.02, abs: 2.0 });
    expectApprox(ctx, 'major principal stress', solverVal(values, 'lambda1'), { rel: 0.03, abs: 0.6 });
    expectApprox(ctx, 'minor principal stress', solverVal(values, 'lambda2'), { rel: 0.03, abs: 0.6 });
    expectApprox(ctx, 'principal-axis angle', solverVal(values, 'thetaDeg'), { rel: 0.03, abs: 0.4 });
    expectApprox(ctx, 'equivalent stress', solverVal(values, 'sigmaEq'), { rel: 0.03, abs: 0.6 });
  });

export const verifyQ46: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ46();
    expectApprox(ctx, 'propagator magnitude', solverVal(values, 'kMag'), { rel: 0.04 });
    expectApprox(ctx, 'path-integral phase', solverVal(values, 'phi'), { rel: 0.03, abs: 0.05 });
    expectApprox(ctx, 'classical action', solverVal(values, 'sClassical'), { rel: 0.04 });
    expectApprox(ctx, 'effective classical velocity', solverVal(values, 'vClassical'), { rel: 0.03 });
    expectApprox(ctx, 'effective kinetic energy in eV', solverVal(values, 'eClassicalEv'), { rel: 0.04, abs: 0.05 });
  });

export const verifyQ47: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ47();
    expectApprox(ctx, 'pipe average velocity', solverVal(values, 'v'), { rel: 0.03, abs: 0.02 });
    expectApprox(ctx, 'Reynolds number', solverVal(values, 'reynolds'), { rel: 0.04 });
    expectApprox(ctx, 'friction head loss', solverVal(values, 'hLoss'), { rel: 0.04, abs: 0.01 });
    expectApprox(ctx, 'pressure drop', solverVal(values, 'deltaP'), { rel: 0.04, abs: 50 });
    expectApprox(ctx, 'wall shear stress', solverVal(values, 'tauW'), { rel: 0.05, abs: 0.2 });
    expectApprox(ctx, 'pump power requirement', solverVal(values, 'pumpPower'), { rel: 0.04, abs: 0.2 });
  });

export const verifyQ48: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ48();
    expectApprox(ctx, 'initial dx/dt', solverVal(values, 'dxdt'), { rel: 0.02, abs: 0.02 });
    expectApprox(ctx, 'initial dy/dt', solverVal(values, 'dydt'), { rel: 0.02, abs: 0.05 });
    expectApprox(ctx, 'initial dz/dt', solverVal(values, 'dzdt'), { rel: 0.03, abs: 0.05 });
    expectApprox(ctx, 'nontrivial fixed-point amplitude', solverVal(values, 'eq'), { rel: 0.03, abs: 0.05 });
    expectApprox(ctx, 'phase-space divergence', solverVal(values, 'divergence'), { rel: 0.03, abs: 0.1 });
    expectApprox(ctx, 'Lyapunov doubling time', solverVal(values, 'tDouble'), { rel: 0.05, abs: 0.03 });
  });

export const verifyQ49: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ49();
    expectApprox(ctx, 'Schwarzschild radius in km', solverVal(values, 'rsKm'), { rel: 0.02, abs: 0.2 });
    expectApprox(ctx, 'ISCO radius in km', solverVal(values, 'rIscoKm'), { rel: 0.02, abs: 0.3 });
    expectApprox(ctx, 'photon sphere radius in km', solverVal(values, 'rPhotonKm'), { rel: 0.02, abs: 0.2 });
    expectApprox(ctx, 'ISCO speed as fraction of c', solverVal(values, 'vFracC'), { rel: 0.03, abs: 0.005 });
    expectApprox(ctx, 'gravitational redshift', solverVal(values, 'z'), { rel: 0.03, abs: 0.005 });
    expectApprox(ctx, 'weak-field deflection angle (rad)', solverVal(values, 'alphaRad'), { rel: 0.04, abs: 0.01 });
  });

export const verifyQ50: PhysicsVerificationSolver = (input) =>
  withSolverContext(input, (ctx) => {
    const { values } = solveQ50();
    expectApprox(ctx, 'Wien peak wavelength', solverVal(values, 'lambdaMax'), { rel: 0.03 });
    expectApprox(ctx, 'peak photon energy in eV', solverVal(values, 'photonEnergyEv'), { rel: 0.03, abs: 0.03 });
    expectApprox(ctx, 'Planck exponent x at 500nm', solverVal(values, 'x'), { rel: 0.03, abs: 0.05 });
    expectApprox(ctx, 'Planck radiance B_lambda', solverVal(values, 'bLambda'), { rel: 0.08 });
    expectApprox(ctx, 'Stefan-Boltzmann flux', solverVal(values, 'flux'), { rel: 0.04 });
    expectApprox(ctx, 'blackbody luminosity', solverVal(values, 'luminosity'), { rel: 0.04 });
  });
