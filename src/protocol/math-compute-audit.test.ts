...
describe('independent numeric audit (computed, not pattern-faked)', () => {
  ...
  it('Q37 Newton divided difference gives P(1.5)=4.75', () => {
    ...
    const p1: [number, number] = [1, 3];
    const p2: [number, number] = [2, 7];
    const p3: [number, number] = [3, 13];
    const f01 = (p1[1] - p0[1]) / (p1[0] - p0[0]);
    const f12 = (p2[1] - p1[1]) / (p2[0] - p1[0]);
    const f23 = (p3[1] - p2[1]) / (p3[0] - p2[0]);
    const f012 = (f12 - f01) / (p2[0] - p0[0]);
    const f123 = (f23 - f12) / (p3[0] - p1[0]);
    const f0123 = (f123 - f012) / (p3[0] - p0[0]);
    const x = 1.5;
    const p =
      p0[1] +
      f01 * (x - 0) +
