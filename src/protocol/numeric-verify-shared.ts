/**
 * Independent numeric checks against AI-generated capsule text.
 * Extracts numbers from @body/@formula/@solution — not pre-authored answers.
 */
...
export type NumericVerificationSolver = (input: SolverInput) => VerificationResult;
...
function extractFractions(text: string): number[] {
  ...
  const plainFrac = /(?<![A-Za-z0-9.])([+-]?\d+)\s*\/\s*([+-]?\d+)(?![A-Za-z0-9/])/g;
  while ((match = plainFrac.exec(text)) !== null) {
    const numerator = Number.parseFloat(match[1] ?? '');
    const denominator = Number.parseFloat(match[2] ?? '');
    if (Number.isFinite(numerator) && Number.isFinite(denominator) && denominator !== 0) {
      out.push(numerator / denominator);
