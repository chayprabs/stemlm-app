...
top of **Google Gemini** (gemini.google.com). **stemLM has no backend server and
no account/login.** Your questions and the AI's answers are processed entirely
inside your browser.
...
answer Gemini returns _on the page you're already on_, parses it into steps, and
displays it in the side panel. This processing happens locally in the page. This
content is **not** transmitted to stemLM — it only ever reaches Google (Gemini),
exactly as it would without stemLM.
...
reflected in the repository and the "last updated" date above.
